...
const Desktop = (webrtc, state, emitter) => {
  ...
  const reset = () => {
    ...
    isReady = false;
    pairedMobiles.forEach(
      pairedMobile => (pairedMobile = createPairedMobile())
    );
  };
  ...
  const tryStarting = peerValues => {
    pairedMobiles.forEach(pairedMobile => {
      console.log(
        `tryStarting: pairedMobile.id: ${
          pairedMobile.id
        } pairedMobile.isReady: ${pairedMobile.isReady}`
      );
      if (pairedMobile.id && !pairedMobile.isReady) {
        ...
        for (let peer of peerValues) {
          if (pairedMobile.id === peer.id) {
            console.log(
              `pairedMobile id ${pairedMobile.id} === peer id ${
                peer.id
              }`
            );
  ...
  const pairedMobileLeftRoom = id =>
    getPairedMobile(id)
      ? (pairedMobiles[
          findIndex(pairedMobiles, { id })
        ] = createPairedMobile())
      : null;

  const isValidMobileKey = (key, value) =>
    pairedMobiles.filter(pairedMobile => pairedMobile[key] === value)
      .length === 1;

  const getPairedSecretUsed = secret =>
    !!secret ? findIndex(pairedMobiles, { secret }) > -1 : false;

  const getPairedMobile = id =>
    pairedMobiles.filter(pairedMobile => pairedMobile.id === id)[0];

  const acceptStreams = () =>
    pairedMobiles.filter(({ isReady }) => !isReady).length;

  const needsToForceMobile = () =>
    pairedMobiles.length > 1 &&
    pairedMobiles.filter(pairedMobile => !!pairedMobile.el).length >
      1;
  ...
  const createCanvasStream = () => {
    ...
    logSuccess(
      `Created canvas stream (createCanvasStream) ${
        stream.id
      }. localStreams length: ${webrtc.webrtc.localStreams.length}`
    );
  ...
  const stopPairedMobile = id => {
    if (!id) {
      pairedMobiles.forEach(
        pairedMobile => (pairedMobile.isReady = false)
      );
    } else {
  ...
  const setRemoteDesktopStream = (el, id) => {
    ...
    logInfoB(
      `setRemoteDesktopStream() - targetKey: ${
        remoteDesktopVideo.targetKey
      }`
    );
  };
      ...
      switch (data.type) {
        ...
        */
        case "local:mobile:handshake": {
          ...
          logInfo(
            `local:mobile:handshake ${
              data.from
            } & secret: ${secret} pairedMobiles.length: ${
              pairedMobiles.length
            }`
          );
          console.log(pairedMobiles);
          pairedMobiles.forEach(pairedMobile => {
            console.log(
              `Got mobile handshake from  ${
                data.from
              } with secret ${secret} acceptStreams()?: ${acceptStreams()}.
            ...
            if (!peerIds.get(data.from)) {
              ...
              console.log(
                `\t peerIds was set ${data.from} ${JSON.stringify(
                  Array.from(peerIds.values())
                )}`
              );
            }
            console.log(
              "getPairedSecretUsed(secret)",
              getPairedSecretUsed(secret)
            );
            if (secret && pairedMobile.secret) {
              console.log(
                `\t has payload ${secret} and local secret ${
                  pairedMobile.secret
                }`
              );
              if (secret === pairedMobile.secret) {
                ...
                logSuccess(
                  `localMobile peer id : ${pairedMobile.id}`
                );
              ...
              cancelStreamFromPeer(
                findPeer(peers.values(), data.from)
              );
        ...
        */
        case "local:mobile:joiningRoom": {
          if (
            isValidMobileKey("secret", data.payload.secret) &&
            isValidMobileKey("id", data.from)
          ) {
            logInfo(
              `Mobile ${data.from} is joining ${
                data.payload.roomId
              } so joing too`
            );
  ...
  webrtc.on("videoAdded", function(video, peer) {
    ...
    if (foundPeer && foundPeer.stopStream) {
      ...
      logInfo(
        `videoAdded: connected to another desktop, webcam started?: ${
          webcam.started
        }`
      );
